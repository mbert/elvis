...
#if OS2
	/* get initial video state; turn on ANSI */
	v_vio_init();
#endif
#if VMS
...
#endif
#if OS2
	v_vio_restore();	/* restore initial video state */
#endif
}
...
void suspend_curses()
{
#if ANY_UNIX && !(UNIXV || COH_386 || hpux)
	struct tchars	tbuf;
# ifdef TIOCSLTC
...
#if 0 /* shouldn't need to disable ^Q/^S */
 		oldstart = tbuf.t_startc;
 		tbuf.t_startc = 0;		/* disable ^Q */
 		oldstop = tbuf.t_stopc;
 		tbuf.t_stopc = 0;		/* disable ^S */
#endif
		ioctl(2, TIOCSETC, (struct sgttyb *) &tbuf);
